...
module Spec.Tutorial.Governance2(tests, doVoting, tf, testTree, prop_Gov,
...
import Data.Data
import Plutus.Contract (Contract, selectList, EmptySchema)
...
data Phase = Initial | Establishing | Proposing | Voting | Wait | Finish deriving (Eq, Show, Data)
...
instance ContractModel GovernanceModel where
  ...
  nextReactiveState slot = do
    ...
    when (((slot + 1) >= deadline) && (s == Voting)) $ phase .= Wait  
    when ((slot >= deadline) && (s == Wait)) $ phase .= Finish  
...
check_propGovernanceWithCoverage = do
  ...
  writeCoverageReport "Governance" Gov.covIdx cr
